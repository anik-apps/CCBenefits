...
def test_create_user_card(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1, "nickname": "My Amex"})
...
def test_create_user_card_invalid_template(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 999})
    assert resp.status_code == 404


def test_list_user_cards(client):
    client.post("/api/user-cards/", json={"card_template_id": 1})
    client.post("/api/user-cards/", json={"card_template_id": 2})
    resp = client.get("/api/user-cards/")
    assert resp.status_code == 200
    assert len(resp.json()) == 2


def test_get_user_card_detail(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
    resp = client.get(f"/api/user-cards/{uc_id}")
    assert resp.status_code == 200
    detail = resp.json()
...
def test_user_card_404(client):
    resp = client.get("/api/user-cards/999")
    assert resp.status_code == 404


def test_delete_user_card(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
    resp = client.delete(f"/api/user-cards/{uc_id}")
    assert resp.status_code == 204
    resp = client.get(f"/api/user-cards/{uc_id}")
    assert resp.status_code == 404


def test_delete_user_card_cascades_usage(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
    client.post(f"/api/user-cards/{uc_id}/usage", json={"benefit_template_id": 1, "amount_used": 10.0})
    resp = client.delete(f"/api/user-cards/{uc_id}")
    assert resp.status_code == 204
...
def test_log_continuous_usage(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
...
def test_log_binary_usage_coerces_to_max(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
...
def test_log_binary_usage_zero_stays_zero(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
    equinox_id = _get_benefit_id_by_name(client, 1, "Equinox Credit")
    resp = client.post(
...
def test_usage_exceeds_max_rejected(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
    uber_id = _get_benefit_id_by_name(client, 1, "Uber Cash")
    resp = client.post(
...
def test_duplicate_usage_rejected(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
    uber_id = _get_benefit_id_by_name(client, 1, "Uber Cash")
    client.post(f"/api/user-cards/{uc_id}/usage", json={"benefit_template_id": uber_id, "amount_used": 10.0})
    resp = client.post(f"/api/user-cards/{uc_id}/usage", json={"benefit_template_id": uber_id, "amount_used": 5.0})
    assert resp.status_code == 409


def test_usage_wrong_card_benefit_rejected(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
    ...
    resp = client.post(f"/api/user-cards/{uc_id}/usage", json={"benefit_template_id": lyft_id, "amount_used": 5.0})
    assert resp.status_code == 400


def test_usage_with_target_date(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
    uber_id = _get_benefit_id_by_name(client, 1, "Uber Cash")
    resp = client.post(
...
def test_update_usage(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
    uber_id = _get_benefit_id_by_name(client, 1, "Uber Cash")
    resp = client.post(f"/api/user-cards/{uc_id}/usage", json={"benefit_template_id": uber_id, "amount_used": 10.0})
    usage_id = resp.json()["id"]
    resp = client.put(f"/api/usage/{usage_id}", json={"amount_used": 15.0})
    assert resp.status_code == 200
    assert resp.json()["amount_used"] == 15.0


def test_delete_usage(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
    uber_id = _get_benefit_id_by_name(client, 1, "Uber Cash")
    resp = client.post(f"/api/user-cards/{uc_id}/usage", json={"benefit_template_id": uber_id, "amount_used": 10.0})
    usage_id = resp.json()["id"]
    resp = client.delete(f"/api/usage/{usage_id}")
    assert resp.status_code == 204


def test_update_usage_404(client):
    resp = client.put("/api/usage/999", json={"amount_used": 5.0})
    assert resp.status_code == 404


def test_delete_usage_404(client):
    resp = client.delete("/api/usage/999")
    assert resp.status_code == 404
...
def test_perceived_value_calculation(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
    uber_id = _get_benefit_id_by_name(client, 1, "Uber Cash")

    client.put(f"/api/user-cards/{uc_id}/benefits/{uber_id}/setting", json={"perceived_max_value": 12.0})
    client.post(f"/api/user-cards/{uc_id}/usage", json={"benefit_template_id": uber_id, "amount_used": 10.0})

    detail = client.get(f"/api/user-cards/{uc_id}").json()
...
def test_perceived_value_defaults_to_max(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
    uber_id = _get_benefit_id_by_name(client, 1, "Uber Cash")
...
def test_negative_perceived_value_rejected(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
    uber_id = _get_benefit_id_by_name(client, 1, "Uber Cash")
    resp = client.put(f"/api/user-cards/{uc_id}/benefits/{uber_id}/setting", json={"perceived_max_value": -10.0})
    assert resp.status_code == 422


def test_setting_upsert(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
    uber_id = _get_benefit_id_by_name(client, 1, "Uber Cash")
    ...
    resp = client.put(f"/api/user-cards/{uc_id}/benefits/{uber_id}/setting", json={"perceived_max_value": 5.0})
    assert resp.status_code == 200
...
def test_summary_basic(client):
    ...
    client.post(f"/api/user-cards/{uc_id}/usage", json={"benefit_template_id": uber_id, "amount_used": 15.0})

    resp = client.get(f"/api/user-cards/{uc_id}/summary")
    assert resp.status_code == 200
    summary = resp.json()
...
def test_summary_with_perceived_value(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
    uber_id = _get_benefit_id_by_name(client, 1, "Uber Cash")

    client.put(f"/api/user-cards/{uc_id}/benefits/{uber_id}/setting", json={"perceived_max_value": 10.0})
    client.post(f"/api/user-cards/{uc_id}/usage", json={"benefit_template_id": uber_id, "amount_used": 15.0})

    summary = client.get(f"/api/user-cards/{uc_id}/summary").json()
...
def test_summary_404(client):
    resp = client.get("/api/user-cards/999/summary")
    assert resp.status_code == 404
...
def test_period_segments_monthly(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
    detail = client.get(f"/api/user-cards/{uc_id}").json()
...
def test_period_segments_quarterly(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
    detail = client.get(f"/api/user-cards/{uc_id}").json()
...
def test_period_segments_semiannual(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
    detail = client.get(f"/api/user-cards/{uc_id}").json()
...
def test_period_segments_annual(client):
    resp = client.post("/api/user-cards/", json={"card_template_id": 1})
    uc_id = resp.json()["id"]
    detail = client.get(f"/api/user-cards/{uc_id}").json()
